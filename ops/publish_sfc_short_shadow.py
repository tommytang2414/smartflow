"""Publish a verified isolated SFC short shadow snapshot to exact S3 paths."""
...
SOURCE_DATABASE = Path("/home/ubuntu/SmartFlow-shadow/data/sfc-short-v2-shadow.db")
S3_BUCKET = "smartflow-tommy-db"
S3_KEY = "beta/sfc-short-v2-shadow.db"
ARCHIVE_PREFIX = "snapshots/sfc-short-v2"
...
def _validate_snapshot(snapshot_path: Path) -> dict:
    ...
    try:
        ...
        raw_without_children = connection.execute(
            ...
            "AND (raw.source_event_id NOT LIKE 'sfc_short_report:%' "
            "OR NOT EXISTS ("
    ...
    if {row[0] for row in source_rows} != {"sfc_short"}:
        raise RuntimeError("snapshot is not isolated to SFC short positions")
    if raw_without_children:
        raise RuntimeError("snapshot contains rejected or incomplete SFC evidence")
    if (
        health is None
        or health[0] != "healthy"
        ...
        raise RuntimeError("SFC short source health is not publishable")
    return manifest
...
def publish_snapshot(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="smartflow-sfc-short-") as directory:
        snapshot_path = Path(directory) / "sfc-short-v2-shadow.db"
        ...
        if size_bytes > MAX_SNAPSHOT_BYTES:
            raise RuntimeError("SFC short snapshot exceeds the approved size limit")
        ...
        if hkt_date.day == 1:
            archive_key = (
                ...
                f"sfc-short-v2-shadow-{hkt_date:%Y%m%d}.db"
            )
            archive = {
                "key": archive_key,
                **_put_object(
